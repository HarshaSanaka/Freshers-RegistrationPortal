...
from .forms import AdminStudentCreationForm, StudentLoginForm
...
def admin_dashboard_view(request):
    students = StudentProfile.objects.select_related('user', 'academics').prefetch_related('user__documents').order_by('-created_at')
        ...
        if form.is_valid():
            with transaction.atomic():
                user = User.objects.create_user(
                    ...
                    password=form.cleaned_data['date_of_birth'].strftime('%Y-%m-%d'),
                )
...
def update_status(request, pk, status):
    ...
    if status in ['VERIFIED', 'REJECTED', 'UNDER_REVIEW']:
